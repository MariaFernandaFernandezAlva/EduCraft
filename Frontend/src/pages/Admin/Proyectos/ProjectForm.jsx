...
export default function ProjectForm({ formData, setFormData, errors, onSubmit, loading, isEditing = false }) {
  // Estados locales para los dos "agregadores".
  // Son locales porque son texto temporal del input,
  // no forman parte del proyecto que se va a guardar.
    ...
    <form onSubmit={onSubmit} className="bg-white rounded-lg shadow p-8 space-y-6">
      ...
      <div>
        ...
        {formData.images.length > 0 ? (
          <div className="grid grid-cols-2 md:grid-cols-3 gap-3">
            {formData.images.map((ruta, index) => (
              <div key={ruta} className="relative border border-gray-200 rounded-lg overflow-hidden bg-gray-50">

                {/* Vista previa: si la ruta está mal, no se ve nada.
                    Esa es toda la validación posible sin backend. */}
                <img src={ruta} alt={`Imagen ${index + 1}`} className="h-28 w-full object-cover" />
        ...
        <p className="text-xs text-gray-500 mt-2">
          Sube los archivos a <code>public/images/projects/</code> y escribe aquí la ruta, empezando con <code>/</code>.
        </p>
      </div>
